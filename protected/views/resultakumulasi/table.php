<?php
    $qrow = "SELECT tr.model, tr.bagian_proses, mi.nama_identifikasi, tr.kategori, tr.status, SUM(tr.qty) AS JUMLAH FROM transaksi tr LEFT JOIN master_identifikasi mi ON tr.identifikasi = mi.kode_identifikasi LEFT JOIN master_bagian mb ON tr.bagian_proses = mb.code_master_proses$kondisi GROUP BY tr.model, tr.bagian_proses, tr.bagian_proses, tr.identifikasi, tr.kategori, tr.status";
...
<table class="table table-striped">
    ...
    <tbody>
        <?php
            ...
            if($totalrow > 0){
                $query = "SELECT mb.keterangan_master_proses, tr.model, tr.bagian_proses, mi.nama_identifikasi, tr.kategori, tr.status, SUM(tr.qty) AS JUMLAH FROM transaksi tr LEFT JOIN master_identifikasi mi ON tr.identifikasi = mi.kode_identifikasi LEFT JOIN master_bagian mb ON tr.bagian_proses = mb.code_master_proses$kondisi GROUP BY tr.model, tr.bagian_proses, tr.bagian_proses, tr.identifikasi, tr.kategori, tr.status ORDER BY $sortby $sort limit $start_from, $record_per_page";
        ...
        <tr>
            ...
            <td><?php echo $array[]=$row['status']; ?></td>
             <td><?php echo $array[]=$row['JUMLAH']; ?></td>
